...
export const LinkCard = ({link}) => {
    return (
        <div className="container justify-content-center pt-5">
            <div className="card justify-content-center w-100 pt-5 text-center">
                <div className="card-body">
                    <h5 className="card-title">
                        <svg className="bi bi-link-45deg" width="3em" height="3em" viewBox="0 0 16 16"
                    ...
                    <a href={link.to} className="card-link" target='_blank' rel='noopener noreferrer'>shortened</a>
                    <a href={link.from} className="card-link" target='_blank' rel='noopener noreferrer'>original</a>
                    <p className="card-text">
                        clicks: <strong>{link.clicks}</strong></p>
                    <QRCode value={link.to} />
                    <p className="card-text">date: <strong>{new Date(link.date).toLocaleDateString()}</strong></p>
                </div>
            </div>
        </div>
    )
